...
func (s *Server) handleWHIP(c *gin.Context) {
	...
	s.mu.Lock()
	if previous, exists := s.publishers[streamID]; exists && previous.Connection != nil {
		_ = previous.Connection.Close()
	}
	s.publishers[streamID] = publisher
	s.mu.Unlock()

	s.logger.Info("WHIP publisher connected", zap.String("stream_id", streamID), zap.String("pc_id", pcID))
	response := SDPOffer{
...
func (s *Server) handleWHEP(c *gin.Context) {
	...
	s.logger.Info("WHEP viewer connected", zap.String("stream_id", streamID), zap.String("pc_id", pcID))
	response := SDPOffer{
		Type: pc.LocalDescription().Type.String(),
		SDP:  pc.LocalDescription().SDP,
	}
...
func (s *Server) handleDeleteSession(c *gin.Context) {
	...
	if strings.Contains(path, "/whip/") {
		if pub, exists := s.publishers[streamID]; exists && pub.ID == sessionID {
			...
			s.logger.Info("WHIP session terminated", zap.String("stream_id", streamID), zap.String("session_id", sessionID))
		}
		...
		if viewers, exists := s.viewers[streamID]; exists {
			for i, viewer := range viewers {
				if viewer.ID == sessionID {
					...
					s.logger.Info("WHEP session terminated", zap.String("stream_id", streamID), zap.String("session_id", sessionID))
					break
				}
			}
		}
